      ...
      <div className="w-full overflow-x-auto">
        <div className="overflow-hidden min-w-max">
          <div className={`grid ${gridCols} py-4 text-sm font-medium text-gray-100 bg-slate-300 rounded-md border border-black`}>
            ...
            {comparisonDevices.map((sw) => (
              <div key={sw.id} className="flex items-center py-4">
                <div className={`${sw.status === "Suporte" ? "bg-green-500" : "bg-red-600"} w-3 h-3 rounded-full`}></div>
                <div className="rounded-lg p-2 w-full">
                  <div className="font-bold text-2xl underline">{sw.modelo}</div>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={group}>
                ...
                {properties.map(({property, label}) => (
                  <div
                    ...
                    {comparisonDevices.map((sw) => (
                      <div key={sw.id}>
                        <p className="font-bold ">{sw[property]}</p>
                      </div>
                    ))}
                  </div>
                ))}
...
export default SwitchModalCompare;
